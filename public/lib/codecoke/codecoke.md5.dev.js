...
Hexmd5.prototype = {
  'md5'(s,l){return l?this.hex_md5(s).substr(8,16):this.hex_md5(s);}
  ...
  ,'rstr_md5'(s){return this.binl2rstr(this.binl_md5(this.rstr2binl(s),s.length*8))}
  ,'binl2rstr'(s){
  ...
  ,'binl_md5'(x, len) {
    ...
    for (let i=0;i < xlen; i += 16) {
      ...
      a_4 = [a,b,c,d];
      a_4.forEach((v,i) => {a_4[i] = this.safeAdd(v,o_4[i]);});
    }
    return a_4;
  }
  ,'safeAdd'(x,y){let lsw=(x&0xFFFF)+(y&0xFFFF),msw=(x>>16)+(y>>16)+(lsw>>16);return(msw<<16)|(lsw&0xFFFF);}

  ,'bit_rol'(num,cnt){return(num<<cnt)|(num>>>(32-cnt));}
  ,'_cmn'(q,a,b,x,s,t){return this.safeAdd(this.bit_rol(this.safeAdd(this.safeAdd(a,q),this.safeAdd(x,t)),s),b);}
