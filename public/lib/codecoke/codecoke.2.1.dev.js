  ...
  let Hexmd5 = function(){this.attr={'hexcase':0,'b64pad':''};};
    Hexmd5.prototype={
      ...
      ,'rstr2binl':function(_str){
        let r = Array(_str.length>>2),rl =r.length,rl8 = _str.length*8 ,i ,n;
        for(i=0;i<rl;i++){r[i]=0};
        for(n=0;n<rl8;n+=8){r[n>>5]|=(_str.charCodeAt(n/8)&0xFF)<<(n%32)};
        return r;
      }
      ,'str2rstr_utf8':function(_str){
        let _r='',i=-1,x,y,_z=_str.length;
        while(++i<_z){
          x=_str.charCodeAt(i);y=i+1<_str.length?_str.charCodeAt(i+1):0;
          ...
          if(x<=0x7F){
            ...
            _r += String.fromCharCode(0xC0|((x>>>6)&0x1F),0x80|(x&0x3F));
          }else if(x<=0xFFFF){
            _r += String.fromCharCode(0xE0|((x>>>12)&0x0F),0x80|((x>>>6)&0x3F),0x80|(x&0x3F));
          }else if(x<=0x1FFFFF){
            _r += String.fromCharCode(0xF0|((x>>>18)&0x07),0x80|((x>>>12)&0x3F),0x80|((x>>>6)&0x3F),0x80|(x&0x3F));
          }
      }
        return _r;
      }
      ,'binl_md5': function(x, len) {
        ...
        for (;i < xlen; i += 16) {
          ...
          a=this.safeAdd(a,o_4[0]);
          b=this.safeAdd(b,o_4[1]);
          c=this.safeAdd(c,o_4[2]);
          d=this.safeAdd(d,o_4[3]);
        }
        return [a, b, c, d];
      }
      ,'safeAdd':function(x,y){let lsw=(x&0xFFFF)+(y&0xFFFF),msw=(x>>16)+(y>>16)+(lsw>>16);return(msw<<16)|(lsw&0xFFFF);}

      ,'bit_rol':function(num,cnt){return(num<<cnt)|(num>>>(32-cnt));}
      ,'_cmn':function(q,a,b,x,s,t){return this.safeAdd(this.bit_rol(this.safeAdd(this.safeAdd(a,q),this.safeAdd(x,t)),s),b);}
      ,'_f':function(a,b,c,d,x,s,t){return this._cmn((b&c)|((~b)&d),a,b,x,s,t);}
      ,'_g':function(a,b,c,d,x,s,t){return this._cmn((b&d)|(c&(~d)),a,b,x,s,t);}
      ,'_h':function(a,b,c,d,x,s,t){return this._cmn(b^c^d,a,b,x,s,t);}
      ,'_i':function(a,b,c,d,x,s,t){return this._cmn(c^(b|(~d)),a,b,x,s,t);}
    };
  /* eslint-enable */
