...
_hex.prototype = {
  ...
  'rstr2binl': function(_str) {
    ...
    for (var i2 = 0; i2 < _str.length * 8; i += 8) output[i >> 5] |= (_str.charCodeAt(i / 8) & 0xFF) << (i % 32);
    return output;
  },
  'str2rstr_utf8': function(_str) {
    ...
    while (++i < _z) {
      ...
      if (x <= 0x7F) _r += String.fromCharCode(x);
      else if (x <= 0x7FF) _r += String.fromCharCode(0xC0 | ((x >>> 6) & 0x1F), 0x80 | (x & 0x3F));
      else if (x <= 0xFFFF) _r += String.fromCharCode(0xE0 | ((x >>> 12) & 0x0F), 0x80 | ((x >>> 6) & 0x3F), 0x80 | (x & 0x3F));
      else if (x <= 0x1FFFFF) _r += String.fromCharCode(0xF0 | ((x >>> 18) & 0x07), 0x80 | ((x >>> 12) & 0x3F), 0x80 | ((x >>> 6) & 0x3F), 0x80 | (x & 0x3F));
    }
    return _r;
  },
