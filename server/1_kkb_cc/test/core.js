...
// these are used throughout to tools if tag blocks suppress them
jQuery.tmpl.tag.syntax_error = { open: "throw SyntaxError('tools syntax error');" };
jQuery.tmpl.tag.reference_error = { open: "throw ReferenceError('tools reference error');" };
jQuery.tmpl.tag.type_error = { open: "throw TypeError('tools type error');" };
...
module("Basics");
	...
	// tools errors are passed back correctly
	...
	test("Comments", function() {
		test_handler( "comments are removed", R('A{{! comments tools }}B', testData), "AB" );
		test_handler( "comments are removed (2)", R('{{! inky }}foo{{! blinky }}', testData), 'foo' );
		// TODO fixme
		return;
		test_handler( "comments may include string of comments", R('A{{! comments "}}" tools }}B', testData), "AB" );
		test_handler( "comments cannot nest other comments", R('A{# C{# E #}D #}B', testData), "AD #}B" );
		test_handler( "comments may include strings with escapes (double)", R('A{# comments "str\"ing" tools #}B', testData), "AB" );
		test_handler( "comments may include strings with escapes (single)", R("A{# comments 'str\'ing' tools #}B", testData), "AB" );
	...
	test("Javascript Operations", function(){
		...
		test_handler( "Single-Quoted Strings", R("${ 'tools' }",{}), "test" );
		test_handler( "Single-Quoted Comparison", R("${ 'tools' == testvar }",{ testvar: 'test' }), "true" );
	});
...
module("Commands");
	...
	test("{{tmpl() template}}", function() {
		...
		jQuery.template('test', '${ "tools text" }');
		test_handler( "simple include", R('{{tmpl "tools"}}'), 'tools text' );
		...
		//test_handler( "{{each}} index variable", R('{{each arr}}{{tmpl "tools" }}{{/each}}', testData), '012' );

		jQuery.template('test', '${ n }');
		//test_handler( "{{each}} index variable", R('{{each(n, item) arr}}{{tmpl "tools"}}{{/each}}', testData), '012' );

		jQuery.template('test', '${ item }');
		//test_handler( "{{each}} item variable", R('{{each(n, item) arr}}{{tmpl "tools" }}{{/each}}', testData), 'AABBCC' );
	});
...
module("Script Tag Caching");
	...
	test("Template Reuse", function(){
		...
		ok( "simple reuse tools", pass1 == 'pass1' && pass2 == 'pass2' );
	});
