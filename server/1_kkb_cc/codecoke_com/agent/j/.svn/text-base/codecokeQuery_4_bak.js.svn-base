...
//=======*/
if(!window.codecoke) {
window['codecoke']={'softcreatebyurl':'http://www.codecoke.com/','_BasicFuncObject':{}};
	codecoke.is={
		'Null':function(a){
			return a===null;
		}
		...
		,'Mail':function(a) {
			//1@1.cn
			return (
				a && (typeof(a)==='string')	&& (a.indexOf('@')>0) && (a.length>4) &&
		...
		,'EmptyObj': function( a ) {
			for ( var a1 in a ) {return false;}return true;
		}
		,'ObjWin':function(a){
			return (typeof(a)==='object' && Object.prototype.toString.apply(a) === '[object Window]');
		}
		,'Regexp':function(a){
			return (typeof(a)==='object' && Object.prototype.toString.apply(a) === '[object RegExp]');
		}
		,'TimeDir':function(a) {
			//(!_v || _v.length !==8 || !/^(?:19|20)[0-9]{2}(?:[0][1-9]|1[012])(?:0[1-9]|[12][0-9]|3[01])$/.tools(_v))
		...
		,'InBadwords':function(a,_t) {
			...
			var _badwordsO=$siteInfo && $siteInfo.badwords && codecoke.is.Obj($siteInfo.badwords) ? $siteInfo.badwords : null;
			if(!_badwordsO) {
				return false;
			}
			if(_t) {
...
codecoke.get= {
		...
		,'bindFn':function (_o,fun) {
			var _args = arguments.length>2 ? Array.prototype.slice.call(arguments,2):[];
			return function() {fun.apply(_o,_args)};
		}
		...
		,
		'encodeUri': function percentEncode(s) {
			if (s == null) {
			    return '';
			}
		 ...
		 ,
		'decodeUri': function decodePercent(s) {
			if (s != null) {
		...
		,'guiden' : function(_o) {
			...
			return (_sp && 37<_o)
				? '{'+['A','B','C','D','E','F','G','H','J','K'][parseInt(Math.random()*10)] +_str.toUpperCase() +'}'
				: ['A','B','C','D','E','F','G','H','J','K'][parseInt(Math.random()*10)]+''+ _str.toUpperCase()
				;
		}
		,'time': function(_o) {
...
codecoke.get.declareClassByArr([
{
	'_basic_':{
		...
		,'fn':function(_doc) {
				this.doc = _doc;
				this.el_parent = this.doc.el_iwallpaper.length ? this.doc.el_iwallpaper : $('body');
				this.attr={
					'winid':'win_paper_all'
	...
	,'add':function(_el,_barShow,_JustShowMeOnly) {
		this.creatPaperWin();
		var _paper_id ='paper_' +(_el.attr('id') || codecoke.get.guiden()),_me = $('#'+_paper_id);
		...
		if (!_me.length ){
			...
			!_me.children().length && _me.append(_el);
		}
		_me.hasClass('iSkip') && _me.removeClass('iSkip');
		_barShow && this.el_bar.hasClass('iSkip') && this.el_bar.removeClass('iSkip');
		!_barShow && !this.el_bar.hasClass('iSkip') && this.el_bar.addClass('iSkip');
...
,
{
	...
	,'str':function(_stat,_err,_acinfo) {
			...
			return (_acinfo ||'')+'<strong>错误提示</strong>（'+_stat+' '+_err+'）：'+( this.ajaxErrO[_err2] || '未知错误！');
	}
}
,
{
...
,
{
	'_basic_':{
		...
		,'fn':function(_doc) {
			...
			codecoke.get.newFunIn(this,'msg','_doc_ihead_msg',this);
...
,
{
	'_basic_':{
		...
		,'fn':function(_doc) {
			this.doc =_doc;
			this.el=$('#iFoot');
			//this.copyrightEl=$('#copyrightUrl');
			this.poweredEl=$('#powered');
			this.nav=$('#footNav');
	...
	*/
	,'getQueryObj':function() {
			...
			if (_n>1) {
				...
				if (codecoke.is.Str(arguments[1])) {
					_o.url=arguments[1];
				}
				if (_n>2) {
					...
					if (_n>3 && undefined==_o.cache && codecoke.is.Boolean(arguments[_n-3])) {
							_o.cache=arguments[_n-3];
					}

				}
			}
			if (undefined==_o.cache) { _o.cache=true; }
			_o.args=_o.args || [];
			var _n1 = _k.indexOf('--'),_n2,_n3,_n4,_n5;
			...
			if (_n3>0) {
				...
				_o.dataType=this.defaultDataType;
			}
			_n4 = _k.indexOf('@');
			if (_n4>0) {
	...
	,'errwrite':function(_err,_notIsKstr) {
		this.doc.ihead.msg.write((_notIsKstr ? _err : this.errLive[_err]),'red','add')
	}
	,'get':function() {
			var _o;
			if (codecoke.is.Obj(arguments[0])) {
				...
				_o = this.getQueryObj.apply(this,arguments);
			}

			//alert('kstr:'+_o.kstr+'\n _o.p: '+_o.p+'\n _o.itopk: '+_o.itopk+'\n_o.c:'+_o.c);
			if (this.errLive[_o.kstr]) {
				_o.error
					? _o.error(this.errLive[_o.kstr])
	...
	,'_loaditem':function(_o) {
			if (this.errLive[_o.kstr]) {
					 this.errwrite(this.errLive[_o.kstr],'red','add')
					return false;
			}
			//alert(this.loadLive[_o.kstr]+'\nloadlive')
			if (undefined==this.loadLive[_o.kstr]) {
				this.loadLive[_o.kstr]=0;
				if (!_o.url) {
					//_o.url='/codecoke_com/api/in/get_js/?f='+_o.kstr;
					_o.url=this.apipath+'?gt=_js&f='+_o.kstr;
				}else if (_o.url.indexOf('&')==0) {
					//_o.url ='/codecoke_com/api/in/get_js/?f='+_o.kstr+_o.url
					_o.url =this.apipath+'?gt=_js&f='+_o.kstr+_o.url
				}else if (_o.url.indexOf('gt=')==0) {
					_o.url =this.apipath+'?'+_o.url+'&f='+_o.kstr;
				}
				//alert('_loaditem\n_o.url:'+_o.url)
				var _this=this;
				...
				$.ajax({
					...
					,'error':function  (_xhr,_err) {
						_this.errLive[_o.kstr]= doc.ajaxerr.str(_xhr.status,_err,'jsload_err_'+_this.loadLive[_o.kstr]+'_【'+_o.kstr.replace(/_/g,'.')+'】');
						_o.error
							? _o.error(_this.errLive[_o.kstr]+' <span class="cGray mrR5">url</span>'+this.url)
					...
					,'success':function(_d,_stat){
						if (_o.dataType=='json') {
							_o.args=[_d].concat(_o.args);
						}
						//_this.loadLive[_o.kstr] && (delete _this.loadLive[_o.kstr]);
						_this['_callafterload_'+_o.dataType](_o,true);
					}
				});
			}else {
				this.loadLive[_o.kstr]++;
				if (this.loadLive[_o.kstr]>50) {
					alert('jsload allow max:'+this.loadLive[_o.kstr])
				}else {
					setTimeout(	codecoke.get.bindFn(this,this['_call_'+_o.dataType],_o)	,this.loadLive[_o.kstr]*20);
				}
		...
		if (!_o.itop || !_o.itop[_o.p] || (_o.c && !_o.itop[_o.p][_o.c] ) ) {
			...
			if (!_o.fn) {
						if (_o.c && codecoke.is.Func(_o.itop[_o.p][_o.c])) {
								...
								this.errLive[_o.kstr]='jsload_err(notLoad...): 【'+_o.kstr+'】,but not find func【'+_o.p+'.'+(_o.c||'')+'】';
								 this.errwrite(_o.kstr);
								return false;
						}
			}else {
	...
	,'_callafterload_script':function(_o) {
		...
		(undefined != this.loadLive[_o.kstr]) && (delete this.loadLive[_o.kstr]);
		if (!_o.itop || !_o.itop[_o.p] || (_o.c && !_o.itop[_o.p][_o.c] ) ){
					this.errLive[_o.kstr]='jsload_err(hasload...):【'+_o.kstr+'】,but not find 【'+_o.itopk+(_o.p? '.'+_o.p:'')+(_o.c?'.'+_o.c:'')+'】';
					_o.error
						? _o.error(this.errLive[_o.kstr])
						:  this.errwrite(_o.kstr)
					;
		...
		if (codecoke.is.Func(_o.itop)) {
			...
			this.errLive[_o.kstr]='jsload_err(hasload...):【'+_o.kstr+'】,but not find 【'+_o.itopk+'.'+_o.p+'.'+(_o.c||'')+'】';
			//this.doc.ihead.msg.write(this.errLive[_o.kstr],'red','add');
			this.errwrite(_o.kstr);
			return false;
		}
	}
	,'_call_json':function(_o) {
				if (this.errLive[_o.kstr] || !_o.fn) {
						var _errstr = this.errLive[_o.kstr] || 'jsload_err:【'+_o.kstr.replace(/_/g,'.')+'】need fun for json';
						_o.error
							? _o.error(_errstr)
