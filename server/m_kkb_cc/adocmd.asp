...
var _r =GC.DB.ado.prRowsExe({
	'prsArr':[
		['userID','int',4,5389]
		,['myUserID','int',4,5389]
	]
	,'prName':'prGetUserInfo'
});
var _r3 =GC.DB.ado.prRowsExe({
	'prsArr':[
		['userID','int',4,7]
		,['myUserID','int',4,7]
	]
...
if (!_r.err) {
	Response.Write(_r.prs.rNum+'<hr>'+_r.icount+'<hr>'+_r.rows+'<hr>'+_r.rows[0][1])
}else {
	Response.Write(_r.err);
}
if (!_r2.err) {
	Response.Write(_r2.prs.rNum+'<hr>'+_r2.icount+'<hr>'+_r2.rows+'<hr>'+_r2.rows[0][1])
}else {
	Response.Write(_r2.err);
}

//_prName,_prArrs,_datalink


Response.Write('<br/>'+(_r1.err||_r1.prs.userID))
Response.Write('<br/>'+(_r3.err||_r3.prs.userID))
/*
