...
codecoke.fn.declareClass({
	'_basic_':{
		'name':'C_server_icookie_action'
		,'fn':function(_top) {
			this.attr={};
			this.attr.name = _top.attr.name || (GC.quest.Q('web_rootdomain') || 'coc').substr(0,3)+'_unknow';
	...
	,'child_get':function(_n) {
		...
		_n= this.icookie.replaceCookieNameSpecialWords(_n);
		var _n1,_k;
		...
		if ( (new RegExp('(?:^|&)'+_n1+'\=([^&]*?)(?:&|$)') ).test(this._cookiesValuesString)) {
			...
			this.d['_'+_n1] =this.d['_'+_n1] ?(_k?(_k ==1? GC.iEscape.uni(this.d['_'+_n1]):GC.iEscape.un(this.d['_'+_n1],this.attr.iEscapeCookiekey))
	...
	,'chile_setSingle':function(_n,_v,_notSetIndata) {
		...
		_n = this.icookie.replaceCookieNameSpecialWords(_n.replace(/\s+/g,''));
		var _n1 ,_k,_v1,_newV;
		if (this.attr.nameAttr[_n]) {
				_n1 = this.attr.nameAttr[_n][0];
				_k =this.attr.nameAttr[_n][1];
			}else {
		...
		//_v = this.attr.keyAttr[_n]?(this.attr.keyAttr[_n] ==1? GC.iEscape.dei(_v): GC.iEscape.de(_v,this.attr.iEscapeCookiekey)):_v.replace(/&+/g,'＆');
		_v1 = _k?(_k ==1? GC.iEscape.dei(_v): GC.iEscape.de(_v,this.attr.iEscapeCookiekey)):_v.replace(/&+/g,'＆');
		_newV = '='+_v1;
		if (!this.readstr()){
